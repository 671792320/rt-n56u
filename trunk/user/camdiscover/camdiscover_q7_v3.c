...
static void print_dev(const char *type,const char *ip,long n){char k[96];snprintf(k,sizeof(k),"%s:%s",type,ip);if(!seen_add(k))return;printf("DEVICE type=%s IP=%s MAC=- INFO=%s response_len=%ld\n",type,ip,type,n);fflush(stdout);}
...
static void rx(int fd,const char*kind){unsigned char b[BUF];struct sockaddr_in s;socklen_t sl=sizeof(s);ssize_t n;char ip[64];n=recvfrom(fd,b,sizeof(b),0,(struct sockaddr*)&s,&sl);if(n<=0)return;if(!inet_ntop(AF_INET,&s.sin_addr,ip,sizeof(ip)))return;print_dev(kind,ip,(long)n);printf("[camdiscover] %s RX %s:%d bytes=%ld\n",kind,ip,ntohs(s.sin_port),(long)n);fflush(stdout);}

int main(int argc,char**argv){struct ctx c;const char*ifname="br0";int opt,maxfd,rc,i;struct timeval tv;fd_set rfds;time_t end;unsigned char frame[256];memset(&c,0,sizeof(c));c.onvif=c.ssdp=c.hik=c.dahua=-1;c.timeout=8;c.onvif_en=c.ssdp_en=c.hik_en=c.dahua_en=1;c.onvif_port=3702;c.ssdp_port=1900;c.hik_port=37020;c.dahua_port=37810;while((opt=getopt(argc,argv,"i:t:o:s:k:d:O:S:H:D:A:C:I:"))!=-1){switch(opt){case'i':ifname=optarg;break;case't':c.timeout=atoi(optarg);break;case'o':c.onvif_port=atoi(optarg);break;case's':c.ssdp_port=atoi(optarg);break;case'k':c.hik_port=atoi(optarg);break;case'd':c.dahua_port=atoi(optarg);break;case'O':c.onvif_en=atoi(optarg)!=0;break;case'S':c.ssdp_en=atoi(optarg)!=0;break;case'H':c.hik_en=atoi(optarg)!=0;break;case'D':c.dahua_en=atoi(optarg)!=0;break;case'I':if(inet_aton(optarg,&c.src))c.has_src=1;break;case'A':break;case'C':break;default:break;}}
if(c.timeout<1)c.timeout=1;if(c.timeout>30)c.timeout=30;c.ifindex=ifindex_of(ifname);if(!c.has_src)c.has_src=load_src(&c.src);load_subnets(&c);printf("[camdiscover] iface=%s ifindex=%d source=%s target_subnets=%d\n",ifname,c.ifindex,c.has_src?inet_ntoa(c.src):"auto",c.subnet_count);if(c.onvif_en)c.onvif=make_rx(&c.src,c.has_src,ONVIF_G,c.onvif_port);if(c.ssdp_en)c.ssdp=make_rx(&c.src,c.has_src,SSDP_G,c.ssdp_port);if(c.hik_en)c.hik=make_rx(&c.src,c.has_src,HIK_G,c.hik_port);if(c.dahua_en)c.dahua=make_rx(&c.src,c.has_src,DAHUA_G,c.dahua_port);printf("[camdiscover] sockets ONVIF=%d SSDP=%d HIK=%d DAHUA=%d\n",c.onvif,c.ssdp,c.hik,c.dahua);fflush(stdout);
if(c.onvif>=0)printf("[camdiscover] ONVIF multicast %s\n",sendto_addr(c.onvif,ONVIF_G,c.onvif_port,onvif_probe,strlen(onvif_probe))==0?"sent":"FAILED");if(c.ssdp>=0)printf("[camdiscover] SSDP multicast %s\n",sendto_addr(c.ssdp,SSDP_G,c.ssdp_port,ssdp_probe,strlen(ssdp_probe))==0?"sent":"FAILED");if(c.hik>=0){printf("[camdiscover] HIK multicast %s\n",sendto_addr(c.hik,HIK_G,c.hik_port,hik_probe,strlen(hik_probe))==0?"sent":"FAILED");for(i=0;i<c.subnet_count;i++){char bc[64];bc_addr(c.subnet[i],bc);if(bc[0])printf("[camdiscover] HIK broadcast %s %s\n",bc,sendto_addr(c.hik,bc,c.hik_port,hik_probe,strlen(hik_probe))==0?"sent":"FAILED");}}
if(c.dahua>=0){size_t n=dahua_probe(frame,sizeof(frame));printf("[camdiscover] DAHUA multicast %s\n",sendto_addr(c.dahua,DAHUA_G,c.dahua_port,frame,n)==0?"sent":"FAILED");for(i=0;i<c.subnet_count;i++){char bc[64];bc_addr(c.subnet[i],bc);if(bc[0])printf("[camdiscover] DAHUA broadcast %s %s\n",bc,sendto_addr(c.dahua,bc,c.dahua_port,frame,n)==0?"sent":"FAILED");}}
