...
static int nvram_number(const char *name, int fallback){
    FILE *f; char buf[32]; int v;
    f=popen("nvram get lan_discovery_probe_timeout 2>/dev/null","r");
    if(!f)return fallback;
    if(!fgets(buf,sizeof(buf),f)){pclose(f);return fallback;}
    pclose(f);
    v=atoi(buf); if(v<1||v>30)return fallback; return v;
}
