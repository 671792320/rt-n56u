...
static struct local_addr local_addr[MAX_LOCAL];
...
static void add_local_ip(const char *ip)
{
    ...
    for (i = 0; i < local_count; i++)
        if (!strcmp(local_addr[i].ip, ip)) return;
    if (local_count < MAX_LOCAL) {
        strncpy(local_addr[local_count].ip, ip, sizeof(local_addr[local_count].ip) - 1);
        local_addr[local_count].ip[sizeof(local_addr[local_count].ip) - 1] = 0;
        local_count++;
    }
}

static void add_local_if(const char *ifname)
{
    int fd;
    struct ifreq ifr;
    char ip[INET_ADDRSTRLEN];
    ...
    if (ioctl(fd, SIOCGIFHWADDR, &ifr) == 0) {
        ...
        if (local_count < MAX_LOCAL)
            strncpy(local_addr[local_count].mac, mac, sizeof(local_addr[local_count].mac) - 1);
        /* MAC is checked separately below; keep it in a simple static slot. */
    }
    close(fd);
}
...
static void collect_if(const char *ifname)
{
    ...
    if (ioctl(fd, SIOCGIFHWADDR, &ifr) == 0) {
        ...
        if (m[0] || m[1] || m[2] || m[3] || m[4] || m[5]) {
            snprintf(local_macs[local_mac_count], sizeof(local_macs[0]),
                     "%02x:%02x:%02x:%02x:%02x:%02x",
                     m[0], m[1], m[2], m[3], m[4], m[5]);
            for (i = 0; i < local_mac_count; i++)
                if (!strcmp(local_macs[i], local_macs[local_mac_count])) { close(fd); return; }
            if (local_mac_count < MAX_LOCAL - 1) local_mac_count++;
        }
    }
    close(fd);
}
...
static int is_local_line(const char *line)
{
    ...
    if (p) {
        ...
        if ((size_t)(q - p) < sizeof(ip)) {
            ...
            for (i = 0; i < local_count; i++)
                if (!strcmp(ip, local_addr[i].ip)) return 1;
        }
    }
    p = strstr(line, " MAC=");
    if (p) {
