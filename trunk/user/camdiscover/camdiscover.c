 ...
 * belongs to the future netmgr component.
 */
#include <arpa/inet.h>
#include <errno.h>
#include <getopt.h>
...
static int send_probe(int fd, const struct sockaddr_in *dst)
{
    static const char probe[] =
        ...
        "<e:Header>"
        "<w:MessageID>uuid:camdiscover-q7</w:MessageID>"
        "<w:To>urn:schemas-xmlsoap-org:ws:2005:04:discovery</w:To>"
        "<w:Action>http://schemas.xmlsoap.org/ws/2005/04/discovery/Probe</w:Action>"
        "</e:Header>"
        "<e:Body><d:Probe><d:Types>dn:NetworkVideoTransmitter</d:Types>"
        "<d:Scopes/></d:Probe></e:Body></e:Envelope>";

    return (int)sendto(fd, probe, sizeof(probe) - 1, 0,
...
int main(int argc, char **argv)
{
    ...
    if (ifname && setsockopt(fd, IPPROTO_IP, IP_MULTICAST_IF,
                             &(struct in_addr){mreq.imr_address},
                             sizeof(struct in_addr)) < 0) {
        /* Some old kernels require an address rather than ifindex here.
         * Membership above is still sufficient for receiving replies. */
    }
