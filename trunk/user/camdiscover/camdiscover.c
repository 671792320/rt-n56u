...
int main(int argc,char**argv){
    ...
    if(dahua)c.fd_dahua=make_udp_receiver(DAHUA_ADDR,dahua_port,c.ifindex);
    if(raw){c.fd_raw=socket(AF_PACKET,SOCK_RAW,htons(ETH_P_ALL));if(c.fd_raw>=0){struct sockaddr_ll s;memset(&s,0,sizeof(s));s.sll_family=AF_PACKET;s.sll_ifindex=(int)c.ifindex;s.sll_protocol=htons(ETH_P_ALL);if(bind(c.fd_raw,(struct sockaddr*)&s,sizeof(s))<0){close(c.fd_raw);c.fd_raw=-1;}}}
