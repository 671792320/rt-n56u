...
function parse_data(data){data=norm(data);var first=(data.split('\n')[0]||'').split('|');return {iface:value_or(first[1],value_or(initial_status.iface,value_or(cfg.ifname,'eth2.1'))),role:value_or(first[2],value_or(initial_status.role,'LAN')),ip:value_or(first[3],value_or(initial_status.ip,value_or(cfg.ip,'-'))),mac:value_or(first[4],value_or(initial_status.mac,value_or(cfg.mac,'-'))),link:value_or(first[5],value_or(initial_status.link,'-')),dhcp:value_or(first[6],value_or(initial_status.dhcp,'未检测')),state:value_or(first[7],value_or(initial_status.state,'空闲')),count:value_or(first[8],value_or(initial_status.count,'0')),last:value_or(first[9],value_or(initial_status.last,'-')),interfaces:section(data,'---IFACES---','---LOG---'),log:section(data,'---LOG---','---DEVICES---'),devices:section(data,'---DEVICES---','---CUSTOM---'),custom:section(data,'---CUSTOM---','')};}
...
function render_interfaces(s){var sel=document.getElementById('lan_ifname');var wanted=value_or(cfg.ifname,'eth2.1');var lines=norm(s).split('\n');var found=false;if(!s)return;sel.innerHTML='';for(var i=0;i<lines.length;i++){var f=lines[i].split('|');if(f.length<5||!f[0])continue;if(f[1]!=='LAN'||/^(lo|br|ra|wds|apcli)/.test(f[0]))continue;var opt=document.createElement('option');opt.value=f[0];opt.text=f[0]+' | '+f[1]+' | '+(f[2]||'-')+' | '+(f[4]||'-');if(f[0]===wanted){opt.selected=true;found=true;}sel.appendChild(opt);}if(!sel.options.length){var opt2=document.createElement('option');opt2.value=wanted;opt2.text=wanted+' | LAN | '+value_or(cfg.ip,'-')+' | UP';opt2.selected=true;sel.appendChild(opt2);}else if(!found)sel.selectedIndex=0;}
function render_devices(s){var body=document.getElementById('devices');body.innerHTML='';var lines=norm(s).split('\n');for(var i=0;i<lines.length;i++){var z=lines[i];if(z.indexOf('DEVICE ')!==0)continue;var type=(z.match(/type=([^ ]+)/)||[])[1]||'-';var ip=(z.match(/IP=([^ ]+)/)||[])[1]||'-';var mac=(z.match(/MAC=([^ ]+)/)||[])[1]||'-';var info=(z.match(/INFO=(.*)$/)||[])[1]||'-';var tr=document.createElement('tr');tr.innerHTML='<td>-</td><td>'+esc(type)+'</td><td>'+esc(ip)+'</td><td>'+esc(mac)+'</td><td>'+esc(info)+'</td>';body.appendChild(tr);}if(!body.children.length)body.innerHTML='<tr><td colspan="5" class="muted">暂无设备</td></tr>';}
function add_custom_row(name,addr,port,payload,en){var id='lan_custom_'+(new Date().getTime())+'_'+Math.floor(Math.random()*100000);var row=document.createElement('div');row.className='custom_row';var on=String(en)==='1';row.innerHTML='<table class="table table-condensed" style="margin:5px 0"><tr><td width="18%"><input class="span12 c_name" value="'+esc(name||'')+'"></td><td width="23%"><input class="span12 c_addr" value="'+esc(addr||'')+'"></td><td width="12%"><input class="span12 c_port" value="'+esc(port||'')+'"></td><td width="27%"><input class="span12 c_payload" value="'+esc(payload||'')+'"></td><td width="12%"><div class="main_itoggle"><div id="'+id+'_on_of"><input type="checkbox" id="'+id+'_fake" '+(on?'checked':'')+'></div></div><div style="position:absolute;margin-left:-10000px"><input type="radio" id="'+id+'_1" name="'+id+'" value="1" '+(on?'checked':'')+'><input type="radio" id="'+id+'_0" name="'+id+'" value="0" '+(!on?'checked':'')+'></div><input type="hidden" class="c_enable" value="'+(on?'1':'0')+'"></td><td width="8%"><button type="button" class="btn btn-danger btn-mini" onclick="remove_custom(this);return false;">删除</button></td></tr></table>';document.getElementById('custom_list').appendChild(row);init_itoggle(id,function(){row.querySelector('.c_enable').value=document.getElementById(id+'_fake').checked?'1':'0';});}
function remove_custom(btn){var row=btn;while(row&&!/(^| )custom_row( |$)/.test(row.className||''))row=row.parentNode;if(row&&row.parentNode)row.parentNode.removeChild(row);}
function render_custom(s){if(custom_loaded||!s)return;custom_loaded=true;var lines=norm(s).split('\n');for(var i=0;i<lines.length;i++){if(!lines[i])continue;var f=lines[i].split('|');if(f.length<5)continue;var n=f[0],a=f[1],p=f[2],y=f[3];try{n=decodeURIComponent(n);a=decodeURIComponent(a);p=decodeURIComponent(p);y=decodeURIComponent(y);}catch(e){}add_custom_row(n,a,p,y,f[4]);}}
function add_custom(){add_custom_row('','','','',1);}
function serialize_custom(){var rows=document.getElementById('custom_list').getElementsByTagName('div');var out=[];for(var i=0;i<rows.length;i++){if(!/(^| )custom_row( |$)/.test(rows[i].className||''))continue;out.push(encodeURIComponent(rows[i].querySelector('.c_name').value)+'|'+encodeURIComponent(rows[i].querySelector('.c_addr').value)+'|'+encodeURIComponent(rows[i].querySelector('.c_port').value)+'|'+encodeURIComponent(rows[i].querySelector('.c_payload').value)+'|'+rows[i].querySelector('.c_enable').value);}document.form.lan_discovery_custom.value=out.join('\n');}
function refresh_data(){var x=new XMLHttpRequest();x.onreadystatechange=function(){if(x.readyState!==4||x.status!==200)return;var o=parse_data(x.responseText);render_status(o);render_interfaces(o.interfaces);render_devices(o.devices);render_custom(o.custom);var lg=document.getElementById('live_log');lg.textContent=o.log||'暂无日志';lg.scrollTop=lg.scrollHeight;};x.open('GET','Advanced_LANDiscover_Data.asp?_='+new Date().getTime(),true);x.send(null);}
function applyRule(){if(!login_safe())return false;serialize_custom();showLoading();document.form.action_mode.value=' Apply ';document.form.current_page.value='Advanced_LANDiscover_Content.asp';document.form.next_page.value='';document.form.submit();return false;}
function clearLog(){if(!login_safe())return false;document.form.lan_discovery_log.value='';applyRule();}
function initial(){show_banner(1);show_menu(5,2,0);show_footer();render_status({iface:value_or(initial_status.iface,value_or(cfg.ifname,'eth2.1')),role:value_or(initial_status.role,'LAN'),ip:value_or(initial_status.ip,value_or(cfg.ip,'-')),mac:value_or(initial_status.mac,value_or(cfg.mac,'-')),link:value_or(initial_status.link,'-'),dhcp:value_or(initial_status.dhcp,'未检测'),state:value_or(initial_status.state,'空闲'),count:value_or(initial_status.count,'0'),last:value_or(initial_status.last,'-')});if(!document.form.lan_discovery_dhcp_timeout.value)document.form.lan_discovery_dhcp_timeout.value='3';if(!document.form.lan_discovery_cycle.value)document.form.lan_discovery_cycle.value='10';if(!document.form.lan_discovery_onvif_port.value)document.form.lan_discovery_onvif_port.value='3702';if(!document.form.lan_discovery_ssdp_port.value)document.form.lan_discovery_ssdp_port.value='1900';if(!document.form.lan_discovery_hik_port.value)document.form.lan_discovery_hik_port.value='37020';if(!document.form.lan_discovery_dahua_port.value)document.form.lan_discovery_dahua_port.value='37810';refresh_data();if(refresh_timer)clearInterval(refresh_timer);refresh_timer=setInterval(refresh_data,1000);}
</script>
<style>.status-table td{white-space:nowrap}.help-tip{cursor:help;color:#888}.mini{width:48px;margin:0 4px}.live-box{height:170px;overflow:auto;background:#111;color:#ddd;padding:8px;font:12px monospace}.custom_row{margin:5px 0}.table th,.table td{vertical-align:middle}</style>
</head>
...
<input type="hidden" name="current_page" value="Advanced_LANDiscover_Content.asp"><input type="hidden" name="next_page" value=""><input type="hidden" name="sid_list" value="LANHostConfig;"><input type="hidden" name="action_mode" value=""><input type="hidden" name="action_script" value=""><input type="hidden" name="lan_discovery_log" id="lan_discovery_log" value=""><input type="hidden" name="lan_discovery_custom" id="lan_discovery_custom" value="">
...
<table class="table table-bordered table-condensed"><tr><th width="180">检测接口</th><td><select name="lan_discovery_ifname" id="lan_ifname" class="span9"><option value="<% nvram_get_x("", "lan_discovery_ifname"); %>" selected><% nvram_get_x("", "lan_discovery_ifname"); %> | LAN</option></select></td></tr>
...
<h4>自定义 UDP 发现协议 <button type="button" class="btn btn-success btn-mini pull-right" onclick="add_custom();return false;">＋ 添加</button></h4><div class="muted">可无限添加；开关使用系统原生样式。</div><div id="custom_list"></div>
