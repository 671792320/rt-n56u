...
def main():
    # Data接口直接使用Padavan原生EJ输出目标网段，避免页面依赖旧NVRAM缓存。
    ...
    # 统一parse_data：必须把TARGETS独立分段解析出来。
    ...
    if "ping:'检测中',backend_status:''" not in page:
        page = replace_once(page, old_record, new_record, '设备记录函数')

    # DEVICE记录解析时读取PROTO/PING/STATUS，供页面真实显示。
    ...
    if "render_target_states(o.targets)" not in page:
        page = replace_once(page, 'render_devices(o.devices);render_log(o.log);', 'render_devices(o.devices);render_target_states(o.targets);render_log(o.log);', '目标状态刷新调用')
    ...
    if 'function render_target_states(s)' not in page:
        ...
        page = replace_once(page, 'function render_custom_hint()', target_function + 'function render_custom_hint()', '目标状态函数位置')
    ...
    if 'id="target_states"' not in page:
        ...
        page = replace_once(page, '<h4>IP占用情况 ', target_html + '<h4>IP占用情况 ', '目标状态区域')

    # 最关键修复：设备表必须真正渲染Ping列，不能只有表头没有数据。
    old_vals = "var vals=[st,protocol,r.ip,displayMac,r.info||'-'];"
    new_vals = "var vals=[st,protocol,r.ip,displayMac,r.ping||'不可用',r.info||'-'];"
    page = replace_once(page, old_vals, new_vals, '设备表数据列')
    ...
    # 最终强校验：任何一项缺失都必须让Q7编译直接失败，不能再生成“半修复”固件。
    checks = [
        ...
        ("var vals=[st,protocol,r.ip,displayMac,r.ping||'不可用',r.info||'-'];", '设备Ping数据列'),
        ('<th>状态</th><th>协议</th><th>IP</th><th>MAC</th><th>Ping</th><th>信息</th>', 'Ping表头'),
    ]
