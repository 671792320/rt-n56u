...
# Keep the existing source patches, but make the generated page deterministic.
def patch_state():
    s = STATE.read_text()
...
def patch_camdiscover():
    ...
    # The current camdiscover source in the branch already contains the stable
    # built-in protocols.  Do not silently rewrite it here; custom UDP support
    # is added only when the source does not already have it.
...
def patch_lan_script():
    s = LAN.read_text()
    # The watcher reads all 64 slots.  Empty/disabled rows are ignored.
    if 'while [ "$i" -le 64 ]; do' not in s:
...
def patch_page():
    # Only render one active custom row initially.  The other 63 rows are
    # hidden but their NVRAM-backed values remain available when editing.
    rows=[]
    for i in range(1,MAX_CUSTOM+1):
        rows.append(f'''<tr id="lan_custom_row_{i}"><td><select name="lan_discovery_custom_{i}_enable"><option value="0" selected>禁用</option><option value="1" <% nvram_match_x("", "lan_discovery_custom_{i}_enable", "1", "selected"); %>>启用</option></select></td><td><input name="lan_discovery_custom_{i}_name" class="span3" value="<% nvram_get_x("", "lan_discovery_custom_{i}_name"); %>"></td><td><input name="lan_discovery_custom_{i}_port" class="span2" value="<% nvram_get_x("", "lan_discovery_custom_{i}_port"); %>"></td><td><input name="lan_discovery_custom_{i}_payload" class="span5" value="<% nvram_get_x("", "lan_discovery_custom_{i}_payload"); %>"></td><td><button type="button" class="btn btn-mini" onclick="lanCustomRemoveRow({i});">−</button></td></tr>''')
    defaults={
        'lan_discovery_enable':'1','lan_discovery_ifname':'eth2.1','lan_discovery_dhcp_enable':'1',
        'lan_discovery_dhcp_timeout':'3','lan_discovery_discover_enable':'1','lan_discovery_timeout':'10',
        'lan_discovery_onvif':'1','lan_discovery_onvif_port':'3702','lan_discovery_ssdp':'1','lan_discovery_ssdp_port':'1900',
        'lan_discovery_hik':'1','lan_discovery_hik_port':'37020','lan_discovery_dahua':'1','lan_discovery_dahua_port':'37810','lan_discovery_raw':'1'
    }
    main_rows=''
    for k,v in defaults.items():
        main_rows += f'<input type="hidden" id="def_{k}" value="{v}">'
    page=f'''<!DOCTYPE html><html><head><title><#Web_Title#> - LAN自动发现</title><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><meta http-equiv="Pragma" content="no-cache"><meta http-equiv="Expires" content="-1"><link rel="shortcut icon" href="images/favicon.ico"><link rel="stylesheet" type="text/css" href="/bootstrap/css/bootstrap.min.css"><link rel="stylesheet" type="text/css" href="/bootstrap/css/main.css"><script src="/jquery.js"></script><script src="/state.js"></script><script src="/general.js"></script><script src="/popup.js"></script><script>\nfunction setDefault(name,v){{var e=document.form[name];if(e&&e.value==='')e.value=v;}}\nfunction initial(){{show_banner(1);var m=(typeof lan_discovery_menu_index=='number'&&lan_discovery_menu_index>=0)?lan_discovery_menu_index:5;show_menu(m,0,0);show_footer();{''.join([f"setDefault(''{k}'',''{v}'');" for k,v in defaults.items()])}lanCustomInit();}}\nfunction save(){{if(typeof login_safe==='function'&&!login_safe())return false;document.form.action_mode.value=' Apply ';document.form.current_page.value='Advanced_LANDiscover_Content.asp';document.form.next_page.value='Advanced_LANDiscover_Content.asp';document.form.submit();}}\nvar lanCustomCount=1;function lanCustomInit(){{var n=1;for(var i=1;i<={MAX_CUSTOM};i++){{var a=document.form['lan_discovery_custom_'+i+'_name'],b=document.form['lan_discovery_custom_'+i+'_port'],e=document.form['lan_discovery_custom_'+i+'_enable'];if((a&&a.value)||(b&&b.value)||(e&&e.value=='1'))n=i;}}lanCustomCount=n;lanCustomSync();}}\nfunction lanCustomSync(){{for(var i=1;i<={MAX_CUSTOM};i++){{var r=document.getElementById('lan_custom_row_'+i);if(r)r.style.display=(i<=lanCustomCount)?'':'none';}}document.getElementById('lan_custom_count').innerHTML=lanCustomCount+'/{MAX_CUSTOM}';}}\nfunction lanCustomAdd(){{if(lanCustomCount<{MAX_CUSTOM}){{lanCustomCount++;var e=document.form['lan_discovery_custom_'+lanCustomCount+'_enable'];if(e)e.value='0';lanCustomSync();}}}}\nfunction lanCustomRemoveRow(i){{if(i<1||i>lanCustomCount)return;for(var j=i;j<lanCustomCount;j++){{['enable','name','port','payload'].forEach(function(k){{var a=document.form['lan_discovery_custom_'+j+'_'+k],b=document.form['lan_discovery_custom_'+(j+1)+'_'+k];if(a&&b)a.value=b.value;}});}}var e=document.form['lan_discovery_custom_'+lanCustomCount+'_enable'];if(e)e.value='0';['name','port','payload'].forEach(function(k){{var b=document.form['lan_discovery_custom_'+lanCustomCount+'_'+k];if(b)b.value='';}});if(lanCustomCount>1)lanCustomCount--;lanCustomSync();}}\nfunction lanCustomRemove(){{lanCustomRemoveRow(lanCustomCount);}}\n</script></head><body onload="initial();"><div class="wrapper"><div class="container-fluid"><div class="row-fluid"><div class="span3"><center><div id="logo"></div></center></div><div class="span9"><div id="TopBanner"></div></div></div></div><div id="Loading" class="popup_bg"></div><iframe name="hidden_frame" id="hidden_frame" src="" width="0" height="0" frameborder="0"></iframe><form method="post" name="form" action="start_apply.htm" target="hidden_frame"><input type="hidden" name="current_page" value="Advanced_LANDiscover_Content.asp"><input type="hidden" name="next_page" value="Advanced_LANDiscover_Content.asp"><input type="hidden" name="sid_list" value="LANDiscovery;"><input type="hidden" name="action_mode" value="">{main_rows}<div class="container-fluid"><div class="row-fluid"><div class="span3"><div class="well sidebar-nav side_nav" style="padding:0"><ul id="mainMenu" class="clearfix"></ul><ul class="clearfix"><li><div id="subMenu" class="accordion"></div></li></ul></div></div><div class="span9"><div class="box well grad_colour_dark_blue"><h2 class="box_head round_top">LAN 自动发现</h2><div class="round_bottom"><div class="alert alert-info">LAN口 Link Up 后先检测 DHCP，再执行设备发现。默认启用；自身 IP/MAC 自动过滤。</div><table width="100%" class="table"><tr><td>LAN自动发现</td><td><select name="lan_discovery_enable"><option value="0">禁用</option><option value="1">启用</option></select></td></tr><tr><td>检测接口</td><td><input name="lan_discovery_ifname" class="span4"></td></tr><tr><td>DHCP检测</td><td><select name="lan_discovery_dhcp_enable"><option value="0">禁用</option><option value="1">启用</option></select> 超时 <input name="lan_discovery_dhcp_timeout" class="span2"> 秒</td></tr><tr><td>设备发现</td><td><select name="lan_discovery_discover_enable"><option value="0">禁用</option><option value="1">启用</option></select> 超时 <input name="lan_discovery_timeout" class="span2"> 秒</td></tr></table><h4>内置协议</h4><table width="100%" class="table"><tr><td>ONVIF</td><td><select name="lan_discovery_onvif"><option value="0">禁用</option><option value="1">启用</option></select> 端口 <input name="lan_discovery_onvif_port" class="span2"></td></tr><tr><td>SSDP</td><td><select name="lan_discovery_ssdp"><option value="0">禁用</option><option value="1">启用</option></select> 端口 <input name="lan_discovery_ssdp_port" class="span2"></td></tr><tr><td>HIK-SADP</td><td><select name="lan_discovery_hik"><option value="0">禁用</option><option value="1">启用</option></select> 端口 <input name="lan_discovery_hik_port" class="span2"></td></tr><tr><td>DAHUA-DHIP</td><td><select name="lan_discovery_dahua"><option value="0">禁用</option><option value="1">启用</option></select> 端口 <input name="lan_discovery_dahua_port" class="span2"></td></tr><tr><td>ARP/IP</td><td><select name="lan_discovery_raw"><option value="0">禁用</option><option value="1">启用</option></select></td></tr></table><h4>自定义 UDP</h4><div class="alert alert-warning">默认 1 条；用“＋新增”增加，用“−删除”删除。最多 64 条存储槽，实际可按需增加。名称、UDP 端口、发送内容均可设置。</div><button type="button" class="btn btn-success" onclick="lanCustomAdd();">＋ 新增</button> <button type="button" class="btn" onclick="lanCustomRemove();">− 删除</button> <span id="lan_custom_count">1/{MAX_CUSTOM}</span><table width="100%" class="table table-condensed"><tr><th>启用</th><th>名称</th><th>UDP端口</th><th>发送内容</th><th></th></tr>{''.join(rows)}</table><button type="button" class="btn btn-primary" onclick="save();">保存设置</button></div></div></div></div></div></div></form><div id="footer"></div></div></body></html>'''
